...
if kubectl get deployment dns-deploy-cka -n dns-ns &> /dev/null; then
    echo "Validation PASSED: ReplicaSet dns-deploy-cka exists."
else
    echo "Validation FAILED: ReplicaSet dns-deploy-cka does not exist."
    flag="true"
fi  

# Check if there are exactly 2 replicas
replica_count=$(kubectl get deployment dns-deploy-cka -n dns-ns -o=jsonpath='{.status.replicas}')
if [ "$replica_count" -eq 2 ]; then
    echo "Validation PASSED: ReplicaSet dns-deploy-cka has 2 replicas."
else
    echo "Validation FAILED: ReplicaSet dns-deploy-cka does not have 2 replicas."
    flag="true"
fi
...
if [ "$container_name" = "dns-container" ]; then
    echo "Validation PASSED: Container name in dns-deploy-cka is dns-container."
else
    echo "Validation FAILED: Container name in dns-deploy-cka is not dns-container."
    flag="true"
fi
...
if [ "$image_name" = "registry.k8s.io/e2e-test-images/jessie-dnsutils:1.3" ]; then
    echo "Validation PASSED: Container image in dns-deploy-cka is correct."
else
    echo "Validation FAILED: Container image in dns-deploy-cka is incorrect."
    flag="true"
fi
...
if [ "$command" = "sleep" ]; then
    echo "Validation PASSED: Command in dns-deploy-cka is set to 'sleep 3600'."
else
    echo "Validation FAILED: Command in dns-deploy-cka is not set to 'sleep 3600'."
    flag="true"
fi
